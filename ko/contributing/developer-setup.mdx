...
Decepticon/
├── src/                    # 핵심 애플리케이션 소스
│   ├── agents/             # 에이전트 정의 (Decepticon, Soundwave, Recon, Exploit, PostExploit)
│   ├── skills/             # 스킬 라이브러리 (ATT&CK 태그된 기법들)
│   ├── tools/              # 도구 통합 (샌드박스, C2, 파일 관리)
│   └── cli/                # Ink 기반 CLI 인터페이스
├── docker/                 # Dockerfile 및 Compose 설정
├── tests/                  # 테스트 스위트
├── docs/                   # 문서 소스
